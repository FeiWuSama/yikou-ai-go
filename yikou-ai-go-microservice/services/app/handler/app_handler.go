...
func (a *AppHandler) getLoginUserVo(ctx context.Context, c *app.RequestContext) (userVo.UserVo, error) {
	sessionId := c.Request.Header.Cookie(constants.UserLoginState)
	if sessionId == nil {
		return userVo.UserVo{}, nil
	}

	decodedSessionId, err := url.QueryUnescape(string(sessionId))
	if err != nil {
		return userVo.UserVo{}, err
	}

	resp, err := a.userRpcClient.GetLoginUserBySessionId(ctx, &kitex_gen.GetLoginUserBySessionIdRequest{
...
func (a *AppHandler) ChatToGenCode(ctx context.Context, c *app.RequestContext) {
	...
	userVo, err := a.getLoginUserVo(ctx, c)
	if err != nil {
...
func (a *AppHandler) DownloadAppCode(ctx context.Context, c *app.RequestContext) {
	...
	loginUserVo, err := a.getLoginUserVo(ctx, c)
	if err != nil {
